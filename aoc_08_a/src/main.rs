    ...
    for (antenna, mut positions) in antennas_positions {
        while !positions.is_empty() {
            ...
            for (y, x) in positions.clone() {
                ...
                if y_position >= 0 && y_position < map.len() as i32 && x_position >= 0 && x_position < map[0].len() as i32 && map[y_position as usize][x_position as usize] != antenna {
                    if !antinodes.contains(&(y_position, x_position)) {
